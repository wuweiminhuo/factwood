                      ...
                      <?php switch($field["type"]): case "num": ?><input type="text" class="text input-medium" name="<?php echo ($field["name"]); ?>" value="<?php echo ($data[$field['name']]); ?>"><?php break;?>
                        ...
                        <?php case "picture": ?><div class="controls uploadrow" title="点击修改图片">
                            ...
                            <div class="upload-img-box" rel="img">
                              <?php if(!empty($data[$field['name']])): ?><div class="upload-pre-item"><img width="120" height="120" src="/weiphp2.0.1202<?php echo (get_cover($data[$field['name']],'path')); ?>"/></div><?php endif; ?>
                            </div>
                          </div>
...
<div id="option_list">
  ...
  <?php if(is_array($option_list)): $i = 0; $__LIST__ = $option_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="add_list">
        ...
        <div class="upload-img-box">
          <?php if(!empty($vo["image"])): ?><div class="upload-pre-item"><img width="120" height="120" src="/weiphp2.0.1202<?php echo (get_cover($vo["image"],'path')); ?>"/></div><?php endif; ?>
        </div>
        </div>
        </td>
        <td>
...
if(weixin_cover_picurl==undefined || weixin_cover_picurl==""){
	$('.weixin-cover-pic').attr('src',"/weiphp2.0.1202/Public/Home/images/no_cover.png");
}else{
...
<script type="text/javascript" src="/weiphp2.0.1202/Public/static/datetimepicker/js/bootstrap-datetimepicker.min.js"></script> 
